...
public class AudioManagerBase : MonoBehaviour
{
    ...
    public void PlaySFX(AudioClip effect, float volume = 1f, float pitch = 1f, bool loop = false)
    {
        ...
        for (int i = allSongs.Count - 1; i >= 0; i--)
        {
            ...
            if (song == activeSong)
            {
                if (song.volume < song.maxVolume)
                    song.volume = Mathf.MoveTowards(song.volume, song.maxVolume, speed);
            ...
            else
            {
                if (song.volume > 0)
                {
                    song.volume = Mathf.MoveTowards(song.volume, 0f, speed);
                    anyValueChanged = true;
                }
                else
                {
    ...
    {
        AudioSource newSource = Instantiate(instance.audioPrefab).GetComponent<AudioSource>();
    ...
    public class SONG
    {
        ...
        {
            source = AudioManagerBase.CreateNewSource(string.Format("SONG [{0}]", clip.name));
            source.clip = clip;
            ...
            AudioManagerBase.allSongs.Add(this);
        ...
        public virtual void DestroySFX()
        {
            AudioManagerBase.allSongs.Remove(this);
