...
/// <summary>
/// Access all commands for the VN System
/// </summary>
...
namespace Command_Library
{
    ...
    /*
        TODO: Implement an active feature such that when a character is speaking, all other characters
        are darkened so the speaking character has the focus. It is clear who is talking.

        TODO: The Fire Emblem game will have a single body, where the facial expressions change over the
       static body. We'll have to decide whether we want to do Persona 5-styled animated facial expressions
        or if we want to keep to the traditional VN with still images only.

        At any rate, both structures will work like this:
            - Face (animated or still)
            - Body (single type. No alterations)

           Only difference is that the animated style will implement an Animator.

        if(data[0] == "setFamiliarity")
            {
                Command_SetFamiliarity(data[1]);
                return;
            }

        void Command_SetFamiliarity(string data)
        {
            string[] parameters = data.Split(',');
            bool hasMet = true;
            if(parameters.Length > 1)
            {
                bool.TryParse(parameters[1], out hasMet);
            }

            bool isEnabled = true;
            if(parameters.Length > 2)
            {
                bool.TryParse(parameters[2], out isEnabled);
            }

            Character character = CharacterManager.instance.GetCharacter(parameters[0], enableCreatedCharacterOnStart: isEnabled);
            character.hasMet = hasMet;
        }
