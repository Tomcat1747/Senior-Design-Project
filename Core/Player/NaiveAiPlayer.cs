...
public class NaiveAiPlayer : Player
{
    ...
    public override void Play(GridSystem gridSystem)
    {
        gridSystem.GridState = new GridStateAiTurn(gridSystem);
    ...
    {
        var myUnits = _gridSystem.Units.FindAll(u => u.PlayerNumber.Equals(PlayerNumber)).ToList();
        foreach (var unit in myUnits.OrderByDescending(u => u.Cell.Neighbors.FindAll(u.IsCellTraversable).Count))
        {
            var enemyUnits = _gridSystem.Units.Except(myUnits).ToList();
            ...
            foreach (var enemyUnit in enemyUnits)
            {
                potentialDestinations.AddRange(_gridSystem.Cells.FindAll(c => unit.IsCellMovableTo(c) && unit.IsUnitAttackable(enemyUnit, c)));
            ...
            {
                var path = unit.FindPath(_gridSystem.Cells, potentialDestination);
            ...
            {
                foreach (var potentialDestination in shortestPath.Intersect(unit.GetAvailableDestinations(_gridSystem.Cells)).OrderByDescending(h => h.GetDistance(unit.Cell)))
                {
                    var path = unit.FindPath(_gridSystem.Cells, potentialDestination);
                    var pathCost = path.Sum(h => h.MovementCost);
                    if (pathCost > 0 && pathCost <= unit.MovementPoints)
