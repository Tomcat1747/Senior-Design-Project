...
public abstract class Unit : MonoBehaviour
{
    ...
    {
        cachedPaths = null;
        Skills.FindAll(b => b.Duration == 0).ForEach(b => { b.Undo(this); });
        Skills.RemoveAll(b => b.Duration == 0);
        Skills.ForEach(b => { b.Duration--; });
    ...
    public virtual void DealDamage(Unit other)
    {
        ...
        if (!IsUnitAttackable(other, Cell))
            return;
        //print (name + "deals " + AttackFactor + " damage!"); // NOTE: Remove when done debugging
    ...
    protected virtual void Defend(Unit other, int damage)
    {
        ...
        int dmg = Mathf.Clamp(damage - DefenceFactor, 1, damage);
        //print (name + "has a Defence Factor of " + DefenceFactor + " and only took " + dmg + " damage!"); // NOTE Remove when done debugging
    ...
    public abstract List<Cell> GetAttackDestinations(List<Cell> cells, HashSet<Cell> _pathsInRange);

    protected Dictionary<Cell, List<Cell>> cachePaths(List<Cell> cells)
    {
    ...
    protected virtual Dictionary<Cell, Dictionary<Cell, int>> GetGraphEdges(List<Cell> cells)
    {
        ...
        foreach (var cell in cells)
        {
            if (IsCellTraversable(cell) || cell.Equals(Cell))
            {
                ...
                foreach (var neighbour in cell.Neighbors.FindAll(IsCellTraversable))
                {
    ...
    /// <summary>
    /// Give visual indication that the unit is avoiding.
    /// </summary>
    /// <param name="other"></param>
    public abstract void MarkAsAvoiding(Unit other);
    /// <summary>
    /// Give visual indication that the unit is hit by a critical hit.
    /// </summary>
    /// <param name="other"></param>
    public abstract void MarkAsCritical(Unit other);
    /// <summary>
